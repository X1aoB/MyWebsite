...
describe("Now 分类与文章路由", () => {
  ...
  it("分类保持指定顺序，技术文章引用既有阅读页且不生成副本", () => {
    ...
    expect(now.sections.map((section) => getNowArticleSummaries(section, posts).length)).toEqual([2, 1, 1, 0]);
    ...
    expect(paths.map((path) => path.params)).toEqual([
      { section: "daily", slug: "japan-trip-preparation" },
      { section: "daily", slug: "typhoon-autumn-recruiting" },
      { section: "interviews", slug: "xiaohongshu-ai-data-interview" }
    ]);
    expect(paths[0].props.item.publishedAt).toBe("2026-09-19T13:32:55+08:00");
    expect(paths[1].props.item.publishedAt).toBe("2026-08-11T16:26:41+08:00");
    expect(paths[2].props.item.publishedAt).toBe("2026-09-03T14:23:32+08:00");
  });
