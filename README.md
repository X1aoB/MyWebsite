...
- 静态摄影集：套图标题卡、按图片比例排列的阅读窗、单图大图预览，以及无需数据库的添加流程。
...
`first-layout-demo.md` 和三张几何图只是首版预览示例；添加第一套真实照片后可替换或删除它。`caption` 与 `note` 可作为内容记录保留，但不会在图片窗口中显示；发布前请自行移除不希望公开的定位信息，并确保照片拥有公开版权或已获得授权。
